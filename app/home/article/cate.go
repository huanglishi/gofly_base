...
func (api *Cate) Get_list(c *gin.Context) {
	list, _ := model.DB().Table("gofly_article_cate").Fields("id,pid,name").Order("id asc").Get()
	if list == nil {
...
func (api *Cate) Save(c *gin.Context) {
	...
	if f_id == 0 {
		delete(parameter, "id")
		addId, err := model.DB().Table("gofly_article_cate").Data(parameter).InsertGetId()
		if err != nil {
			results.Failed(c, "添加失败", err)
		} else {
			if addId != 0 {
				model.DB().Table("gofly_article_cate").
		...
		res, err := model.DB().Table("gofly_article_cate").
			Data(parameter).
			Where("id", f_id).
			Update()
		if err != nil {
...
func (api *Cate) UpStatus(c *gin.Context) {
	...
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	res2, err := model.DB().Table("article_cate").Where("id", parameter["id"]).Data(map[string]interface{}{"status": parameter["status"]}).Update()
	if err != nil {
		results.Failed(c, "更新失败！", err)
	} else {
		msg := "更新成功！"
...
func (api *Cate) Del(c *gin.Context) {
	...
	_ = json.Unmarshal(body, &parameter)
	ids := parameter["ids"]
	res2, err := model.DB().Table("article_cate").WhereIn("id", ids.([]interface{})).Delete()
	if err != nil {
