...
func JwtVerify(c *gin.Context) {
	...
	//具体路径
	var NoVerify_arr []string
	if global.App.Config.App.NoVerify != "" {
		NoVerify_arr = strings.Split(global.App.Config.App.NoVerify, `,`)
	} else {
		NoVerify_arr = make([]string, 0)
	}
	rootPath := strings.Split(c.Request.URL.Path, "/")
	if len(rootPath) > 2 && IsContain(NoVerifyRoot_arr, rootPath[1]) { //不需要token验证-根路径
		return
	} else if IsContain(NoVerify_arr, c.Request.URL.Path) { //不需要token验证-具体路径
		return
	}
